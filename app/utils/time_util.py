...
DT_AWARE = "%b %d, %Y - %I:%M:%S %p %Z"
DT_NAIVE = "%b %d, %Y - %I:%M:%S %p"
...
def compare_datetimes(dt1: datetime, op, dt2: datetime):
	"""Compares two datetime strings without tzinfo conflicts."""
	ops = {
		'>': operator.gt,
		'<': operator.lt,
		'>=': operator.ge,
...
def make_tznaive(dt):
	"""Make a tzaware datetime object naiea."""
	return dt.replace(tzinfo=None)


# for naive: datetime.fromtimestamp()
